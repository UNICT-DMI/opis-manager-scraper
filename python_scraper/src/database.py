...
def insert_department(department) -> int:
    if not _connection:
        raise ValueError(
            "Database non connesso. Chiama connect_to_db() prima.")
...
def insert_insegnamento(insegnamento, corso_internal_id: int) -> int:
    ...
    try:
        ...
        # Attenzione: verifica che questi nomi di colonna corrispondano
        # esattamente alla tua migrazione Laravel "create_insegnamenti_table"
        sql = """
        ...
        # Recuperiamo l'ID interno appena generato da MySQL
        insegnamento_internal_id = cursor.lastrowid
        cursor.close()

        return insegnamento_internal_id
...
def insert_schede_opis(schede_opis: list, insegnamento_internal_id: int):
    ...
    try:
        ...
        # Prendiamo la prima scheda per capire la struttura
        ...
        for scheda in schede_opis:
            ...
            # PARTE FONDAMENTALE: Trasformiamo le liste/dizionari in JSON
            row_tuple = []
            for col in columns:
                valore = s_dict.get(col)
                # Se il valore è una lista o un dizionario (es. le domande della scheda)
                if isinstance(valore, (list, dict)):
                    # Lo trasformiamo in stringa JSON
                    row_tuple.append(json.dumps(valore))
                else:
                    # Altrimenti lo lasciamo così com'è (stringhe, numeri)
